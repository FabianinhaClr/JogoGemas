...
def main():
    print("Bem Vindo ao Jogo Gemas feito exclusivamente por Clara Antonelo, vamos relembrar, você está jogando o jogo Gemas e funciona da seguinte forma junte 3 ou mais gemas numa mesma linha ou coluna para eliminá-las")
    pontos = 0
    ...
    while existem_movimentos_validos(tabuleiro):
        ...
        if comando == "permanecer":
            l1 = int(input("Qual a linha(horizontal) da gema que você deseja mover: "))
            c1 = int(input("Qual a coluna(vertical) da gema que você deseja mover: "))
            l2 = int(input("Qual a linha(horizontal) da gema que você deseja substituir: "))
            c2 = int(input("Qual a coluna(vertical) da gema que você deseja substituir: "))
            print()
            valido = trocar(l1, c1, l2, c2, tabuleiro)
            if valido:
                numero_gemas = eliminar(tabuleiro)
                total_gemas = 0
                while numero_gemas > 0:
            ...
            print("Que pena de esteja indo embora, vou sentir saudades!!")
            return
        else:
...
def trocar(l1, c1, l2, c2, tabuleiro):
    ...
    novo_tabuleiro[l1][c1] = tabuleiro[l2][c2]
    novo_tabuleiro[l2][c2] = tabuleiro[l1][c1]
    if len(identificar_cadeias_horizontais(novo_tabuleiro)) > 0:
        tabuleiro[l1][c1] = novo_tabuleiro[l1][c1]
        tabuleiro[l2][c2] = novo_tabuleiro[l2][c2]
        return True
